...
class WConfigPageDetail : public WWizardPage
{
	...
	void OnInitDialog()
	{
		...
		UBOOL bHighDetailAll = false;

#if 0
		// DX8 OEM is high detail everything (DirectX8)
		bHighDetailAll = true;
#endif
		...
		if( !bHighDetailAll &&
			Driver==TEXT("SoftDrv.SoftwareRenderDevice") || 280.0*1000.0*1000.0*GSecondsPerCycle>1.f )
		{
		...
		//RUNE:
		// K6's reporting just under 128 MB, so lowered these thresholds

		// Sound quality.
		if( !bHighDetailAll &&
			!GIsMMX || GPhysicalMemory < 96*1024*1024 )
		{
			...
			GConfig->SetString( TEXT("Galaxy.GalaxyAudioSubsystem"), TEXT("EffectsChannels"), TEXT("16") );
...
//			GConfig->SetString( TEXT("Botpack.TournamentPlayer"),    TEXT("bNoVoiceTaunts"),  TEXT("true") );
		...
		// Skins.
		if( !bHighDetailAll &&
			(GPhysicalMemory < 96*1024*1024)/* || (DescFlags&RDDESCF_LowDetailSkins)*/ )
		{
			Info = Info + LocalizeGeneral(TEXT("SkinsLow"),TEXT("Startup")) + TEXT("\r\n");
			GConfig->SetString( TEXT("WinDrv.WindowsClient"), TEXT("SkinDetail"), TEXT("Low") );
		}
		else if (!bHighDetailAll &&
			GPhysicalMemory < 224*1024*1024
			|| (Driver==TEXT("OpenGLDrv.OpenGLRenderDevice")))
		{
			Info = Info + LocalizeGeneral(TEXT("SkinsMedium"),TEXT("Startup")) + TEXT("\r\n");
		...
		// World.
		if( !bHighDetailAll &&
			(GPhysicalMemory < 96*1024*1024)/* || (DescFlags&RDDESCF_LowDetailWorld)*/ )
		{
			Info = Info + LocalizeGeneral(TEXT("WorldLow"),TEXT("Startup")) + TEXT("\r\n");
			GConfig->SetString( TEXT("WinDrv.WindowsClient"), TEXT("TextureDetail"), TEXT("Low") );
		}
		else if(!bHighDetailAll &&
			GPhysicalMemory < 224*1024*1024
			|| (Driver==TEXT("OpenGLDrv.OpenGLRenderDevice")))
		{
			Info = Info + LocalizeGeneral(TEXT("WorldMedium"),TEXT("Startup")) + TEXT("\r\n");
		...
		// Resolution.
		if( !bHighDetailAll &&
			(!GIsMMX || !GIsPentiumPro) && Driver==TEXT("SoftDrv.SoftwareRenderDevice") )
		{
			...
			Driver==TEXT("SoftDrv.SoftwareRenderDevice") || (DescFlags&RDDESCF_LowDetailWorld) )
		{
...
/*	if( FirstRun<220 )
	...
	// RUNE: Display ESRB message
	if ( FirstRun==0 && !GIsEditor )
	{
		FString S;
		if( appLoadFileToString( S, TEXT("License.txt") ) )
		{
			ExitSplash();
			WTextScrollerDialog Dlg( TEXT("License"), *S );
			Dlg.DoModal();
			InitSplash(NULL);
		}
	}
