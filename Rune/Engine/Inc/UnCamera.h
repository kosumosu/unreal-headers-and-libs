...
class ENGINE_API UCanvas : public UObject
{
	...
	INT				X, Y;
	FLOAT			AlphaScale;	// RUNE:  STY_AlphaScale alpha
	UFont			*SmallFont, *MedFont, *BigFont, *LargeFont;
	UFont			*RuneMedFont, *CredsFont, *ButtonFont;	//RUNE
	UViewport*		Viewport;
	...
	DECLARE_FUNCTION(execTransformPoint)
...
enum ERenderType
{
	...
	REN_Transparent		= 7,	// All transparent polys
...
enum EViewportShowFlags
{
	...
	SHOW_Crosshairs		= 0x00008000,	// RUNE: Show crosshairs on all selected actors.
	SHOW_Splines		= 0x00010000,	// RUNE: Show splines for interpolation paths.
	SHOW_EventLines		= 0x00020000,	// RUNE: Show event -> tag connections.
};
