...
class			UMesh;
class			UModel;
class			USkelModel; // RUNE Skeletal model
class		UPolys;
class		ULevelBase;
class			ULevel;
class			UPendingLevel;
...
//
// Defines a strike in a weapon Swipe
//
struct ENGINE_API FSwipeHit
{
	// Variables.
	class AActor *Actor;	// Struck Actor
	INT	LowMask;			// LowBits of Hit Joints
	INT HighMask;			// HighBits of Hit Joints
...
struct ENGINE_API FSAction
{
	FLOAT			Delay;					// Delay before executing
	FName			AnimToPlay;				// Animation to play
	FName			EventToFire;			// Event to broadcast
	class USound *	SoundToPlay;			// Sound to play
	UBOOL			bTaskLocked;			// TaskLocked during this action
};
...
// RUNE: Moved from Render.h for more general access
#if STATS
struct ENGINE_API FEngineStats
{
	// Misc.
	INT ExtraTime;
	...
	// SkelStats
	INT SkelTime;
	INT SkelGetFrameTime, SkelSetAnimTime, SkelDeformTime;
	INT SkelRenderTime, SkelSetupTime, SkelDecimateTime, SkelClipTime, SkelLightTime, SkelRasterTime;
	INT SkelCount, SkelPolyCount, SkelJointCount, SkelVertCount, SkelLoadCount;
	INT SkelApprox;

	// ParticleStats
	INT ParticleTime;
	INT ParticleSetupTime;
	INT ParticleRenderTime;
...
extern ENGINE_API FEngineStats GStat;
#endif
...
//cjrfucked	BYTE		Darkness;	// Darkness level (0 - 255).  Higher is darker (RUNE)	
...
//rune	PF_BigWavy 			= 0x00001000,	// Poly has a big wavy pattern in it.
//rune	PF_SpecialPoly		= 0x00001000,	// Game-specific poly-level render control (reuse BigWavy flag)
	PF_AlphaBlend 		= 0x00001000,	// RUNE:  This poly should be alpha blended
	PF_Damage			= 0x00002000,	// RUNE:  This surface damages anything standing upon it
//rune	PF_SmallWavy		= 0x00002000,	// Small wavy pattern (for water/enviro reflection).  RUNE:  Not used
...
//rune	PF_CloudWavy		= 0x00020000,	// Polygon appears wavy like clouds.
	PF_SpecialLit3		= 0x00020000,   // Special Lit Flag #3
...
//rune	PF_Gouraud			= 0x00200000,	// Gouraud shaded.
	PF_SpecialLit2		= 0x00200000,	// Special Lit Flag #2
