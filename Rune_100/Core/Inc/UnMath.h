...
public:
	...
	FQuatern();
	FQuatern(FRotator &euler);					// Construct from Euler Rotator
	FQuatern(FVector axis, FLOAT radians);		// Construct from Axis-Angle

	// Operations
	void Slerp (FQuatern *from, FQuatern *to, FLOAT alpha);
	void Slerp2(FQuatern *from, FQuatern *to, FLOAT alpha);
	void Lerp  (FQuatern *from, FQuatern *to, FLOAT alpha);
	void Normalize();

	// Conversions
	void Euler(FRotator *euler);
	void AxisAngle(FVector *axis, FLOAT *radians);
};
